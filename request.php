...
while(!empty($check)) {
 //echo $check;     
    ...
    $quantity=$data['V_Data'][$i]['quantity'];
     if(isset($V_ID))
      {
        $result = mysql_query("SELECT Daily_rate,Hourly_rate FROM Vehicles WHERE V_ID = '".$V_ID."'");
     	$row = mysql_fetch_row($result);
     	if ($result && mysql_num_rows($result) > 0)
    	{
           $Daily_rate=$row[0];
...
$_SESSION['details'][$i]=array ("location" => $data['V_Data'][$i]['Location'],"V_ID" => $data['V_Data'][$i]['V_ID'],"From_date" => $string,"To_date" => $string2,"From_time" => $string1,"To_time" => $string3,"quantity" => $quantity,"Vamount" => $amount);
